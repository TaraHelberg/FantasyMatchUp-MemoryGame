...
The user should be able to enjoy the game and feel calmed by the background and entertained by the chosen images. As well as Congratulated on wining the game.
 ...
 * Game Page
    ...
    + A Restart Button is clear and easy to find, this will on clicking restarts the game flip count and timer as well as reshuffles the cards.
    ...
    + Flip counter is displayed and counts the number of flips a user takes in a game two cards flipped will only amoutn to one flip as two cards are required for a match
    + Timer is display and vissable the timer starts ont he first card flipped and continues until either the game is won or the restart button is clicked
    + On completeting of the Game a GameWon Modal pop up is display showing the number of flips taken to win the game and the time taken with a Congratulatiosn message and a easily visable button to Play again to exit the Modal pop up . 
...
* Body
    + The body has the main game area and displays the Timer and Flip counter both in black to be visable aginst the background image
    + The 16 Cards have a black baground to show the beautful silver dragons at there best and for great visablility agaisnt the background image, when flipped the background is a dark blue to complement the background image and them of the page and show the flipped side images well.
    + the Re-Start game button and How to play Buttona re below the game cards and are in Black with white text sticking with the theme and for visablity on hover the background changes to the same blue and the background of the flipped cards , carring through the color theme.
    + The Modal pop ups both the How to play and Game Won message are the same in style and color followimg through with the them and keeping a consistant visial image for the user.     
...
* Below is a list of Technologies / Support I have used to build my site.
    ...
    + Jshint to run my Javascipt code though
...
* Testing During development of the pages was done through the project to see how the project looked and felt
* Testing and Debugging of Javascript was done constantly via dev tools console.log as this was my first time using Javascript many where found a lot to do with using capital or forgetting to use id's or classes to connect the function to the html using the assistance of dev tools and google I was able to find the bugs and correct the erros in the code
